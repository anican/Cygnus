# Parser package utilizes Look-ahead (LALR) Context-Free Grammar (CFG) parsing
...
def p_statement(p):
    '''statement : expression SEMICOLON | empty'''
    p[0] = p[1]
...
# modify so that it is four different functions (add, subtract, divide, multiply)
def p_bin_op(p):
    '''expression : expression PLUS expression | expression SUB expression |
                    expression MULTI expression | expression DIV expression'''
...
def p_commaList(p):
    '''commas : param | param COMMA commas'''
    if (len(p) >= 4):
        p[0] = p[3] + [p[1], ]
    else:
        p[0] = [p[1], ]
...
def p_paramUse(p):
    '''paramuse : NAME | constant | empty'''
    p[0] = p[1]

def p_paramList(p):
    '''commas_param : paramuse | paramuse COMMA commas_param'''
    if (len(p) >= 4):
        p[0] = p[3] + [p[1], ]
    else:
        p[0] = [p[1], ]
...
def p_type(p):
    '''type : INTTYPE | FLOATTYPE | CHARTYPE | FUNCTIONTYPE'''
    p[0] = p[1]
...
def p_defaultParam(p):
    '''param : NAME EQUALS constant | paramtype EQ constant'''
    p[0] = ["default", p[1], p[3]]

def p_list(p):
    '''paramlist : LPAREN commas RPAREN | LPAREN RPAREN'''
    if (p[2] != ')'):
        p[0] = ["params", p[2], ]
    else:
        p[0] = None
